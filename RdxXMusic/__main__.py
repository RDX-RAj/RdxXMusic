...
import config
from RdxXMusic import LOGGER, app, userbot
from RdxXMusic.core.call import Rdx
from RdxXMusic.misc import sudo
from RdxXMusic.plugins import ALL_MODULES
from RdxXMusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("RdxXMusic.plugins" + all_module)
    LOGGER("RdxXMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Rdx.start()
    try:
        await Rdx.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("RdxXMusic").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("RdxXMusic").info("Stopping AnonX Music Bot...")
